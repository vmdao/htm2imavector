...
function wkhtmltoimage(input, options, callback) {
  ...
  var args = [wkhtmltoimage.command];
  if (!options.debug) {
    args.push('--quiet');
  }
  ...
  if (options.debug && !(options instanceof Function)) {
    console.log('[node-wkhtmltoimage] [debug] [command] ' + args.join(' '));
  }
  ...
  if (process.platform === 'win32') {
    ...
    // The return code should be that of wkhtmltoimage and not of cat
    // http://stackoverflow.com/a/18295541/1705056
    var child = spawn(wkhtmltoimage.shell, ['-c', args.join(' ') + ' | cat ; exit ${PIPESTATUS[0]}']);
  }
  ...
  child.on('exit', function (code) {
    if (code !== 0) {
      stderrMessages.push('wkhtmltoimage exited with code ' + code);
  ...
  child.stderr.on('data', function (data) {
    ...
    if (options.debug instanceof Function) {
      ...
      console.log('[node-wkhtmltoimage] [debug] ' + data.toString());
    }
  });

  if (options.debugStdOut && !output) {
    throw new Error('debugStdOut may not be used when wkhtmltoimage\'s output is stdout');
  }
  ...
  if (options.debugStdOut && output) {
    child.stdout.on('data', function (data) {
      if (options.debug instanceof Function) {
        ...
        console.log('[node-wkhtmltoimage] [debugStdOut] ' + data.toString());
      }
    });
  }
